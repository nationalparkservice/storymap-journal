  ...
  storyHTML += '</button>';
	...
	function exportNarrative(content)
	{
		...
		content.find('iframe').each(function() {
			...
			if (videoIDYouTube) {
				node.replaceWith('<div class="media media-image"><img alt="video thumbnail" src="https://img.youtube.com/vi/' + videoIDYouTube + '/0.jpg" /></div><i class="media-video-warning">' + strings.printVideoWarning + '</i>');
			}
	...
	function exportMediaBlock(id, media)
	{
		...
		switch(media.type) {
			...
			case "image":
				...
				outHTML += '<div class="media media-image"><img alt="" src="' + checkURLProtocol(tokenizedUrl) + '" /></div>';
				break;
			case "video":
				var videoIDYouTube = getYoutubeId(media.video.url);

				if (videoIDYouTube) {
					outHTML += '<div class="media media-image"><img alt="video thumbnail" src="https://img.youtube.com/vi/' + videoIDYouTube + '/0.jpg" /></div>';
				}
